...
- List PRs merged to `<DEFAULT_BRANCH>` in the last ~24h that close issues
  ...
  With the reference CLI, `npm run sdlc -- sweep` computes the work-list in one read-only shot
...
- **After** processing every listed merge, `npm run sdlc -- sweep --ack` marks them swept.
...
- **Duplicate/overlap search:** with the reference CLI,
  `npm run sdlc -- dup-check "<title or keywords>" --exclude <this-issue#>` ranks open issues
